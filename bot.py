...
# The class that will be used to create the Dicord Client
class MyClient(discord.Client):
    ...
    # This will check if there is a new post and if there is, it will send a message to the channel you want
    @tasks.loop(seconds=300)
    async def check_for_new_post(self):
        profile = instaloader.Profile.from_username(bot.context, os.getenv('USER_TO_SCRAPE'))
        ...
        file = open('old_url.txt', 'r')
        old_url = file.read()
        file.close()
        
        if old_url != post_details['media'] : 
            file = open('old_url.txt', 'w')
            file.write(post_details['media'])
            file.close()    
            ...
            embed=discord.Embed(title="New Post by "+ the_posts['username'], url="https://instagram.com/"+the_posts['username'], description=the_posts['caption'], color=0xdb2777)
